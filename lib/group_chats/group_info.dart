...
import 'package:tp2/group_chats/add_members.dart';
import 'package:tp2/group_chats/create_group/add_members.dart';
...
class _GroupInfoState extends State<GroupInfo> {
  ...
  Future removeMembers(int index) async {
    ...
    await _firestore.collection('groupes').doc(widget.groupId).update({
      "membres": membersList,
    }).then((value) async {
      await _firestore
          .collection('users')
                    ...
                    checkAdmin()
                        ? ListTile(
                            onTap: () => Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (_) => AjouterMembresGroup(
                                  groupChatId: widget.groupId,
                                  name: widget.groupName,
                                  listMembres: membersList,
                                ),
                              ),
                            ),
                            leading: const Icon(
